//Original Shadertoy Link: https://www.shadertoy.com/view/MdfBRX
//http://glslsandbox.com/e#49013.5
Shader "Unlit/WaterColorDot"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
            ...
            vec3 N31(float p) {
               ...
               vec3 p3 = fract(vec3(p,p,p) * vec3(0.1031,0.11369,0.13787));
            ...
            vec3 EnvironmentLights(float i, float t) {
                ...
                vec3 col = lerp(tailLightCol, streetLightCol, fract(n*-65.42));
			...
			fixed4 frag (v2f i) : SV_Target
			{
				 float t = time;
                vec3 col = vec3(0,0,0);
                vec2 uv = i.uv.xy / _Resolution.xy; // 0 <> 1
                
                uv -= .5;
                uv.x *= _Resolution.x/_Resolution.y;
                
               vec2 _mouse = _Mouse.xy;
                ...
                float bumps = lerp(h1, h2, fract(bt))*.1;
