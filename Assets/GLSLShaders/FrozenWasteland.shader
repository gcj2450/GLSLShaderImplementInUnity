//http://glslsandbox.com/e#23802.0
...
Shader "Unlit/FrozenWasteland"
{
	...
	SubShader
	{
		...
		Pass
		{
            ...
            #define sideLookSpeed 0.15

            float height(in vec2 p)
            {
                float h = sin(p.x*.1+p.y*0.3)+sin(p.y*.1-p.x*0.3)*.5;
            ...
            float vine(vec3 p, in float c, in float h)
            {
                ...
                p.x += cos(p.z*.4575)*1.;
                vec2 q = vec2(fmod(p.x, c)-c/2., p.y);
            ...
            {
                vec3 sky = mix(vec3(1,1,1.), vec3(.05, .2, .3), pow(abs(rd.y+.04), .5));
            ...
            {   
                vec2 p = gl_FragCoord.xy/_Resolution.xy-0.5;
                vec2 q = gl_FragCoord.xy/_Resolution.xy;
                p.x*=_Resolution.x/_Resolution.y;
                //vec2 mo = vec2(0);
                vec2 mo = _Mouse.xy  - 0.5;
                mo.x *= 6.28;
                vec3 ro = vec3(0.+smoothstep(0.,1.,tri(time*.06)*2.)*0.1, smoothstep(0.,1.,tri(time*1.5)*3.)*0.08, -time*moveSpeed-140.0);
                ro.y -= camHeight(ro.zx)-.4;
                mo.x += smoothstep(0.7,1.,sin(time*sideLookSpeed))*.5-1.5 - smoothstep(-.7,-1.,sin(time*sideLookSpeed))*.5;
                ...
                // Post...
                col = min(pow(col*1.1,vec3(0.6,0.6,0.6)), 1.0);
