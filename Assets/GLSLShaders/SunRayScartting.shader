...
//http://glslsandbox.com/e#22634.0
...
Shader "Unlit/SunRayScartting"
{
	Properties
	{
		_Resolution ("Resolution", Vector) = (1,1,0,1)
        _Mouse ("Mouse", Vector) = (1,1,0,1)
         _SunPos("SunPos", Vector) = (1,150,10,1)
         turbidity("turbidity", float) =1
         rayleighCoefficient("rayleighCoefficient", float) =1.5
         mieCoefficient("mieCoefficient", float) = 0.50
         mieDirectionalG("mieDirectionalG", float) = 0.70
	}
	SubShader
	{
		...
		Pass
		{
            ...
            //vec3 sunDirection = normalize(_SunPos);
            vec3 sunDirection;
            ...
            const float e  = 2.4718281828459045235360287471352662497757247;
            ...
            {
                vec3 rayleigh = (8.0 * pow(pi, 3.0) * pow(pow(n, 2.0) - 1.0, 2.0)) / (3.0 * N * 
                        pow(primaryWavelengths, vec3(4.0,4.0,4.0)));   // The rayleigh scattering coefficient
            ...
            vec3 totalMie(vec3 primaryWavelengths, vec3 K, float T)
            {
                ...
                return 0.434 * c * pi * pow((2.0 * pi) / primaryWavelengths, vec3(v - 2.0,v - 2.0,v - 2.0)) * K;
            }
            ...
            {
                return sunIntensity * max(0.0, 1.0 - exp(-((pi/1.95 - acos(zenithAngleCos))/steepness)));
            }
            ...
            {
                return fmod(sin(float(seed)*363.5346+ray*674.2454)*6743.4365, 1.0);
            }
            ...
            {   
                ...
                position.y += 0.33;
                position.x+=_Mouse;
                float ang = atan2(position.x, position.y);
            ...
            vec4 CalculateDensity( in vec3 p, in vec3 diffuse, in vec3 ambient )
            {
                ...
                vec4 res = vec4( d,d,d,d );

                // diffuse is here actually
                res.xyz = mix( diffuse * 8.0, vec3(0.5,0.5,0.5), res.x );
                res.a -= f * 1.0;
                return res;
            }
            ...
            vec4 CalculateClouds( in vec3 ro, in vec3 rd, in vec3 color, in float x )
            {
                ...
                for(int i=0; i<64; i++)
                {
                    ...
                    col.rgb += (ambient) * 0.2;
                    col.rgb *= col.a;
                    col.xyz *= pow(col.xyz, vec3(constrast,constrast,constrast));
            ...
            vec3 ToneMap(vec3 color) {
                ...
                float sunfade = 1.0-clamp(1.0-exp(-(_SunPos.z/500.0)),0.0,1.0);
                float tmpv=1.0/(1.2+(1.2*sunfade));
                toneMappedColor = pow(toneMappedColor,vec3(tmpv,tmpv,tmpv));
            ...
            { 
                sunDirection = normalize(vec3(-(_Mouse.x-0.5)*2., -1, (_Mouse.y-0.5)*2.));
                
                // General parameter setup
                vec2 vPos = 2.0*gl_FragCoord.xy/_Resolution.xy - 1.0;                    // map vPos to -1..1
                float t = time*0.0;                                 // time value, used to animate stuff
                float screenAspectRatio = _Resolution.x/_Resolution.y;                    // the aspect ratio of the screen (e.g. 16:9)
                vec3 finalColor = vec3(0.1,0.1,0.1);                                // The background color, dark gray in this case
               ...
               //Camera setup
                ...
                cam.up = vec3(0.3 + sin(time),0.5,1);                                   // The up vector, change to make the camera roll, in world space
                ...
                _SunPos = vec3(0, -1, abs(sin(time*0.0))*0.9+0.1);
                sunDirection = normalize(_SunPos);//vec3(-(_Mouse.x-0.5)*2.*screenAspectRatio, -1, (_Mouse.y-0.5)*2.));    
                vec3 viewDir = cam.rayDir;//normalize(wPos - cameraPos);
                
                //Sun rays
                vec2 po = ( (gl_FragCoord.xy / _Resolution.xy) * 2.0 - 1.0 ) - _SunPos.xz;
                vec3 sunRay = SunRay(po, _Resolution);
            ...
            //  vec3 rayleighAtX = TotalRayleigh(primaryWavelengths) * rayleighCoefficient;
                ...
                sky *= mix(vec3(1.0,1.0,1.0),pow(somethingElse * Fex,vec3(0.5,0.5,0.5)),clamp(pow(1.0-dot(up, sunDirection),5.0),0.0,1.0));
                ...
                vec4 gl_FragColor;
                gl_FragColor.rgb = pow(final, vec3(1.5,1.5,1.5)) + sunRay;
                gl_FragColor.a = 1.0;
                return gl_FragColor;
            }
			...
			{
				//return main(i.uv);
               return vec4( SunRay(i.uv, _Resolution),1);
			}
			ENDCG
		}
	}
