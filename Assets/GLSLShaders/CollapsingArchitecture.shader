	...
	SubShader
	{
		...
		Pass
		{
            ...
            vec3 Ctexture(vec3 p) {
                ...
                if (wall) t = mix(t, vec3(1,1,1), .5);
                return saturate(t);
            }

            void mainImage(out vec4 fragColor, in vec2 fragCoord) {
                // transform screen coordinates
                vec2 uv = fragCoord.xy / _Resolution.xy * 2. - 1.;
                ...
                vec3 rd;// = normalize(vec3(uv, FOV));
                    float yy = radians(180.*(fragCoord.y/_Resolution.y-.5));
                    float xz = radians(360.*(fragCoord.x/_Resolution.x-.5));
                ...
                // add this all up
                fragColor.rgb = (ao * Ctexture(p)) * (ambient + (specular + diffuse) * shadow);
                ...
                // gamma correction
                fragColor = pow(fragColor, vec4(1. / 2.2,1. / 2.2,1. / 2.2,1. / 2.2));
                fragColor.a=1;
            }
			...
			{
                vec4 gl_FragColor;
				mainImage(gl_FragColor,input.uv.xy);
                return gl_FragColor;
			}
			ENDCG
		}
