//http://glslsandbox.com/e#10303.0
Shader "Unlit/Mountains"
{
	...
	SubShader
	{
		...
		Pass
		{
            ...
            const float2x2 rotate2D = float2x2(1.4623, 1.67231, -1.67231, 1.4623);

            // Alternative rotation:-
            // const float2x2 rotate2D = float2x2(1.2323, 1.999231, -1.999231, 1.22);
            ...
            //--------------------------------------------------------------------------
            float Noisevec3float( in vec3 x )
            {
                vec3 p = floor(x);
                vec3 f = fract(x);
                f = f*f*(3.0-2.0*f);
            ...
            //--------------------------------------------------------------------------
            float Noisevec2float( in vec2 x )
            {
                vec2 p = floor(x);
                vec2 f = fract(x);
                f = f*f*(3.0-2.0*f);
            ...
            {
                vec2 res = vec2(Noisevec2float(x), Noisevec2float(x+vec2(4101.03, 2310.0)));
                return res-vec2(.5, .5);
            }
            ...
            {
                vec2 pos = p*0.08;
                float w = (Noisevec2float(pos*.25)*0.75+.15);
                w = 36.0 * w * w;
                ...
                float ff = Noisevec2float(pos*.003);
            ...
            float Map(in vec3 p)
            {
                ...
                #ifdef TREES
                float ff = Noisevec2float(p.xz*1.3)*.8;
            ...
            vec3 TerrainColour(vec3 pos, vec3 normal, float dis)
            {
                ...
                float f = clamp(Noisevec2float(matPos.xz*.05), 0.0,1.0);//*10.8;
                f += Noisevec2float(matPos.xz*.1+normal.yz*1.08)*.85;
                f *= .55;
                ...
                if (normal.y < .5)
                {
                    ...
                    c = clamp(c*c, 0.1, 1.0);
                    f = Noisevec2float(vec2(matPos.x*.09, matPos.z*.095+matPos.y*0.15));
                    f += Noisevec2float(vec2(matPos.x*2.233, matPos.z*2.23))*0.5;
                    mat = mix(mat, vec3(0.4*f,0.4*f,0.4*f), c);
                    specular+=.1;
                }
                ...
                if (matPos.y < 45.35 && normal.y > .65)
                {
                    ...
                    m = vec3(Noisevec2float(matPos.xz*.073)*.5+.15, Noisevec2float(matPos.xz*.12)*.6+.25, 0.0);
                    m *= (normal.y- 0.75)*.85;
                ...
                if (treeCol > 0.0)
                {
                    mat = vec3(.02+Noisevec2float(matPos.xz*5.0)*.03, .05, .0);
                    normal = normalize(normal+vec3(Noisevec2float(matPos.xz*33.0)*1.0-.5, .0, Noisevec2float(matPos.xz*33.0)*1.0-.5));
                    specular = .0;
                }
                #endif
                ...
                {
                    float snow = clamp((matPos.y - 50.0 - Noisevec2float(matPos.xz * .1)*28.0) * 0.035, 0.0, 1.0);
                ...
                if (matPos.y < 1.45)
                {
                    if (normal.y > .4)
                    {
                        f = Noisevec2float(matPos.xz * .084)*1.5;
            ...
            {
                // Gamma first...
                rgb = pow(rgb, vec3(0.45,0.45,0.45));

                #define CONTRAST 1.2
                #define SATURATION 1.12
                #define BRIGHTNESS 1.14
                float brigdot=dot(vec3(.2125, .7154, .0721), rgb*BRIGHTNESS);
                return mix(vec3(.5,.5,.5), mix(vec3(brigdot,brigdot,brigdot), rgb*BRIGHTNESS, SATURATION), CONTRAST);
            }
            ...
            vec4 main(vec2 gl_FragCoord)
            {
                ...
                float h = 0.0;
                float f = 1.0;
                cameraPos.xz = (_Mouse.yx - 0.5) * 500.0;
                for (int i = 0; i < 7; i++)
                {
                    h += Terrain(cameraPos.xz * f) * f;
                    f -= .1;
                }
                camTar.xz = vec2(0,0);
                camTar.y = cameraPos.y = (h*.5)+3.5;
                
                float roll = 0.02*sin(time*.2);
